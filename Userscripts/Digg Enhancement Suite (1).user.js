...
// @version      4.2
// @description  Adds multiple themes (Dark, Liquid Glass, Catppuccin, Classic Digg), a redesigned keyword filter, a "Squarify" mode, a collapsible/lockable sidebar, and a user blocking feature to the Digg Beta.
...
(function() {
    ...
    GM_addStyle(`
        ...
        #des-keyword-list, #des-blocked-user-list { display: flex; flex-wrap: wrap; gap: 8px; flex-basis: 100%; margin-top: 8px; }
        .des-keyword-tag, .des-user-tag { display: flex; align-items: center; background-color: #e9ecef; color: #495057; padding: 6px 10px; border-radius: 16px; font-size: 0.9em; }
        .des-keyword-tag button, .des-user-tag button { background: none; border: none; color: #6c757d; font-size: 1.2em; cursor: pointer; margin-left: 6px; padding: 0; line-height: 1; }
        .des-keyword-tag button:hover, .des-user-tag button:hover { color: #212529; }
        ...
        .des-block-user-btn { background: none; border: none; cursor: pointer; padding: 0 5px; margin-left: 2px; opacity: 0.4; font-size: 16px; line-height: 1; display: inline-block; vertical-align: middle; }
        .des-block-user-btn:hover { opacity: 1; color: #d9534f; }
        .des-user-blocked { display: none !important; }
    ...
    function applyUserFilter() {
        const blockedUsers = GM_getValue('des_blocked_users', []);
        const comments = document.querySelectorAll('section.relative.flex.max-w-full');

        comments.forEach(comment => {
            const userLink = comment.querySelector('a[href^="/@"]');
            if (!userLink) return;

            const username = userLink.href.split('/').pop().toLowerCase();
            if (blockedUsers.includes(username)) {
                comment.classList.add('des-user-blocked');
            } else {
                comment.classList.remove('des-user-blocked');
            }
        });
    }

    function addBlockButtons() {
        // Target headers that have a user link but have not been processed yet
        const commentHeaders = document.querySelectorAll('section.flex.items-center.gap-3:not([data-des-processed="true"])');
        commentHeaders.forEach(header => {
            const userLink = header.querySelector('a[href^="/@"]');
            if (!userLink) return;

            // Mark this header as processed immediately to prevent duplicates
            header.setAttribute('data-des-processed', 'true');

            const username = userLink.href.split('/').pop();

            const blockBtn = document.createElement('button');
            blockBtn.className = 'des-block-user-btn';
            blockBtn.innerHTML = '&#10754;'; // Block icon
            blockBtn.title = `Block @${username}`;
            blockBtn.onclick = (e) => {
                e.stopPropagation();
                const blockedUsers = GM_getValue('des_blocked_users', []);
                if (!blockedUsers.includes(username.toLowerCase())) {
                    blockedUsers.push(username.toLowerCase());
                    GM_setValue('des_blocked_users', blockedUsers);
                    applyUserFilter();
                }
            };

            if(userLink.parentElement?.parentElement) {
                 userLink.parentElement.parentElement.insertAdjacentElement('afterend', blockBtn);
            }
        });
    }
    ...
    function addSettingsUI() {
        ...
        const modalHTML = `
            <div class="des-modal-content">
                ...
                <h3>Blocked Users</h3>
                <div id="des-blocked-user-list"></div>
        ...
        const blockedUserListDiv = modal.querySelector('#des-blocked-user-list');
        ...
        const renderBlockedUsers = () => {
            const blockedUsers = GM_getValue('des_blocked_users', []);
            blockedUserListDiv.innerHTML = '';
            blockedUsers.forEach(user => {
                const tag = document.createElement('span');
                tag.className = 'des-user-tag';
                tag.textContent = `@${user}`;
                const removeBtn = document.createElement('button');
                removeBtn.innerHTML = '&times;';
                removeBtn.title = `Unblock "@${user}"`;
                removeBtn.onclick = () => {
                    const currentBlocked = GM_getValue('des_blocked_users', []);
                    const newBlocked = currentBlocked.filter(u => u !== user);
                    GM_setValue('des_blocked_users', newBlocked);
                    renderBlockedUsers();
                    applyUserFilter();
                };
                tag.appendChild(removeBtn);
                blockedUserListDiv.appendChild(tag);
            });
             if (blockedUsers.length === 0) {
                blockedUserListDiv.innerHTML = '<p style="color: #6c757d; font-size: 0.9em;">No users blocked. Click the icon next to a user\'s name in the comments to block them.</p>';
            }
        };
    ...
    // Debounce the run function to prevent React hydration errors
    let timeoutId;
    const debouncedRun = () => {
        clearTimeout(timeoutId);
        timeoutId = setTimeout(run, 150); // Wait 150ms after the last change
    };
